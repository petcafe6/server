...
function getOne(dbModel, sessionDoc, req) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			// 	if (doc) {
			// 		let obj = doc.toJSON()
			// 		obj.images = obj.images.map(e => `${process.env.PUBLIC_URL}/api/v1/s3/image/show/${e}/${req.query.w || 400}`)
			// 		resolve(obj)
			// 	} else {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			dbModel.pets.paginate(filter, options)
				...
				.catch(reject)
			// .then(result => {
			// 	result.docs = result.docs.map(petDoc => {
			// 		petDoc.listImage = ''
			// 		if ((petDoc.images || []).length > 0) {
			// 			petDoc.listImage = `${process.env.PUBLIC_URL}/api/v1/s3/image/show/${petDoc.images[0]}/${req.query.w || 400}`
			// 		}
			// 		return petDoc
			// 	})
			// 	resolve(result)
